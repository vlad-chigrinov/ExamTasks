...
a.	закрытое поле типа string, в котором хранится имя; 
b.	закрытое поле типа string, в котором хранится фамилия; 
c.	закрытое поле типа System.DateTime для даты рождения. 
b.	В классе Person определить конструкторы: 
a.	конструктор c тремя параметрами типа string, string, DateTime для инициализации всех полей класса;  
b.	конструктор без параметров, инициализирующий все поля класса некоторыми значениями по умолчанию. 
c.	В классе Person определить свойства c методами get и set: 
a.	свойство типа string для доступа к полю с именем;  
b.	свойство типа string для доступа к полю с фамилией; 
c.	свойство типа DateTime для доступа к полю с датой рождения; 
d.	свойство типа int c методами get и set для получения информации(get) и изменения (set) года рождения в закрытом поле типа DateTime, в котором хранится дата рождения. 
d.	В классе Person определить 
a.	перегруженную(override) версию виртуального метода string ToString() для формирования строки со значениями всех полей класса; 
b.	виртуальный метод string ToShortString(), который возвращает строку, содержащую только имя и фамилию. 
...
Методы, позволяющие:
a.	вывести длины сторон треугольника на экран;
b.	рассчитать периметр треугольника;
Свойства:
c.	позволяющее получить-установить длины сторон треугольника (доступное для чтения и записи);
d.	позволяющее установить, существует ли треугольник с данными длинами сторон (доступное только для чтения).
